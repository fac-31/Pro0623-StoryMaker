...
{#if storyboard}
  <div>
    ...
    <p><strong>Outline:</strong> {storyboard.slides?.[storyboard.currentSlide - 1]?.storyOutline}</p>
    ...
    <form on:submit|preventDefault={refineSlide}>
      ...
      <input id="feedback" type="text" bind:value={feedback} placeholder="e.g. Add more action" size="40" />
    ...
    <div style="margin-top:1em;">
      ...
      {#each storyboard.slides as slide}
        <span style="margin-right:0.5em;">{slide.slideNumber}{slide.imageGenerated ? '✅' : ''}</span>
      {/each}
    </div>
...
{#if logs.length}
  <div style="margin-top:2em; background:#222; color:#fff; padding:1em; border-radius:8px; max-height:300px; overflow:auto;">
    <h3>Server Logs</h3>
    <pre>{logs.join('\n')}</pre>
  </div>
{/if}
